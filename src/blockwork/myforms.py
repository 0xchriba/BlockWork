...
class NewProjectForm(forms.Form):
    ...
    Milestone_Descriptions = forms.TypedMultipleChoiceField(choices=[("MILESTONE" + str(x), "MILESTONE" + str(x)) for x in range(1, 11)])

class FindFreelancerForm(forms.Form):
    Project_Skills = forms.TypedMultipleChoiceField(choices=[(x,x) for x in utils.skills])
    Verifcation_methods = forms.TypedChoiceField(choices=[(x,x) for x in utils.verif])
